...
function getinfo($conn)
{
    ...
    $sql = "SELECT h.Name,h.Address,h.Phone,h.Email,h.Fax,h.Password,h.City
    FROM hospital h
    WHERE NOT EXISTS 
    (( SELECT p.PhoneNumber FROM bookedappointment as p ) 
     EXCEPT 
     (SELECT sp.PhoneNumber FROM arranges sp WHERE sp.Address = h.Address )); ";
